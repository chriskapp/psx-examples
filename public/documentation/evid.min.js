...
// Source: ./bower_components/highlightjs/highlight.pack.js
...
// Source: ./build/evid-app.min.js
/*
 evid
 Copyright (C) 2015 Christoph Kappestein
 License: MIT
*/
!function(win){var evid={config:null,links:null,routings:null};evid.loadApi=function(el,selectedTab){var path=$(el).data("path"),version=$(el).data("version");$(".evid-navigation ul li a").each(function(){$(this).removeClass("evid-navigation-selected")}),$(el).addClass("evid-navigation-selected");var html='<div id="evid-resource-toolbar"></div>';html+='<h3 id="evid-resource-title"></h3>',html+='<div id="evid-resource-description"></div>',html+='<div id="evid-resource-nav"></div>',html+='<div id="evid-resource-doc" class="tab-content"><div id="evid-loader"><img /></div></div>',$("#evid-content").html(html);var successCallback=function(resp){if(resp&&"object"==typeof resp&&resp.path){$("#evid-resource-title").html(resp.path+" (v"+resp.resource.version+")"),$("#evid-resource-description").html(resp.description);var html="";if(resp.see_others)for(var key in resp.see_others)html+='<button class="btn btn-default" onclick="evid.goToOther(\''+resp.see_others[key]+"')\">"+key+"</button>";$("#evid-resource-toolbar").html(html),html="";for(var nav='<ul class="nav nav-tabs">',i=0;i<resp.method.length;i++){var method=resp.method[i].toUpperCase();nav+='<li role="presentation"><a href="#'+method+'">'+resp.method[i]+"</a></li>",html+='<div role="tabpanel" class="tab-pane" id="'+method+'">';for(var key in resp.resource.data){var data=resp.resource.data[key],description="",content="";$(data).find(".psx-resource-method").each(function(){$(this).data("method")==method&&(description=$(this).find(".psx-resource-method-description").html(),$(this).find(".psx-resource-data").each(function(){content+='<div class="evid-resource-data">'+$(this).html()+"</div>"}))}),content&&(html+='<div class="evid-resource-method">',description&&(html+='<div class="evid-resource-method-description">'+description+"</div>"),html+="<h4>"+key+"</h4>",html+=content,html+="</div>")}html+="</div>"}nav+="</ul>",$("#evid-resource-nav").html(nav),$("#evid-resource-nav a").click(function(e){e.preventDefault(),$(this).tab("show"),location.hash=resp.path+":"+$(this).attr("href").substr(1)}),$("#evid-resource-doc").html(html),$("#evid-resource-doc").find("pre code").each(function(i,block){hljs.highlightBlock(block)}),"GET"==selectedTab||"POST"==selectedTab||"PUT"==selectedTab||"DELETE"==selectedTab?$('#evid-resource-nav a[href="#'+selectedTab+'"]').trigger("click"):$("#evid-resource-nav a:first").trigger("click")}else location.hash="/"+path,$("#evid-resource-doc").html('<div class="alert alert-info" role="alert">No API documentation available</div>')},errorCallback=function(resp){location.hash="/"+path;var data=resp.responseJSON;$("#resource-doc").html(data.hasOwnProperty("success")&&data.success===!1?'<div class="alert alert-danger" role="alert">'+data.message+"</div><pre>"+data.trace+"</pre>":'<div class="alert alert-danger" role="alert">An unknown error occured</div><pre>'+resp.responseText+"</pre>")};"/"==path.charAt(0)&&(path=path.substr(1));var url=evid.getLink("detail");return null!=url?(url=url.replace(/{version}/g,version),url=url.replace(/{path}/g,path),$.ajax(url,{success:successCallback,error:errorCallback}),!0):!1},evid.loadPage=function(path){$("#evid-content").html('<div id="evid-loader"><img /></div>'),$.ajax({url:path,dataType:"html",success:function(resp){$("#evid-content").html(resp)},error:function(resp){$("#evid-content").html(resp.responseText)}})},evid.getLink=function(rel){if(evid.links)for(var i=0;i<evid.links.length;i++)if(evid.links[i].rel==rel)return evid.links[i].href;return null},evid.getRouting=function(path){if(routings)for(var i=0;i<routings.length;i++)if(routings[i].path==path)return routings[i];return null},evid.goToOther=function(path){window.open(path,"_blank")},evid.initialize=function(){return"object"!=typeof win.evid_config?void $("#evid-content").html('<br><div class="alert alert-danger" role="alert">It looks like the configuration is not available</div>'):"string"!=typeof win.evid_config.url?void $("#evid-content").html('<br><div class="alert alert-danger" role="alert">It looks like the PSX documentation url is not available</div>'):($.isArray(win.evid_config.exclude)||(win.evid_config.exclude=[]),evid.config=win.evid_config,void $.ajax({url:evid.config.url,success:function(resp){if($.isArray(resp.links)&&(evid.links=resp.links),$.isArray(resp.routings)){for(var i=0;i<resp.routings.length;i++)-1==$.inArray(resp.routings[i].path,evid.config.exclude)&&$(".evid-navigation ul").append('<li><a href="#'+resp.routings[i].path+'" data-path="'+resp.routings[i].path+'" data-version="'+resp.routings[i].version+'" onclick="evid.loadApi(this);">'+resp.routings[i].path+"</a></li>");evid.routings=resp.routings}evid.links&&evid.routings?evid.loadFirstApi():$("#evid-content").html('<br><div class="alert alert-danger" role="alert">Looks like the documentation API returns a wrong format</div>')},error:function(){$("#evid-content").html('<br><div class="alert alert-danger" role="alert">Looks like the documentation API returns an error</div>')}}))},evid.loadFirstApi=function(){if(location.hash){var path=location.hash.substr(1),selectedTab=!1;-1!==path.indexOf(":")&&(selectedTab=path.substr(path.lastIndexOf(":")+1),path=path.substr(0,path.lastIndexOf(":")));var el=!1;$(".evid-navigation ul li a").each(function(){el===!1&&$(this).data("path")==path&&(el=this)}),el?evid.loadApi(el,selectedTab):$(".evid-navigation ul li a:first").trigger("click")}else $(".evid-navigation ul li a:first").trigger("click")},win.evid=evid,$(document).ready(function(){evid.initialize()})}(window);